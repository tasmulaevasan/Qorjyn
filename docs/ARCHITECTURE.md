# Архитектура QORJYN Mesh

## Общая схема

QORJYN Mesh разделён на независимый Next.js frontend и Django REST backend. Frontend обращается к API напрямую по `NEXT_PUBLIC_API_URL`. Backend хранит данные через Django ORM и предоставляет единый сервисный слой для HTTP API и WhatsApp-бота.
...
Создание заказа рассчитывает позиции по актуальным ценам поставщика. Переход в `delivered` создаёт складской приход и обновляет рейтинг поставщика. Счётчики поставщика меняются под блокировкой строки.

### WhatsApp

Webhook немедленно подтверждает получение, дедуплицирует сообщения по `idMessage` и передаёт обработку диалоговому сервису. В demo/test режиме отправка сообщений логируется вместо сетевого вызова.
