...
`delta` положительный для прихода и отрицательный для расхода. Отрицательный итоговый остаток запрещён.
...
Ответ `forecast` содержит срок до дефицита, рекомендуемый объём заказа, факторы и семидневный прогноз.
...
Маршрут пересоздаёт demo-данные и не должен оставаться публично доступным без защиты в production.
